...
{
    public partial class UkrTromboeks : UserControl
    {
        private KRTROMBOEKSTOGRAMM _kl;
        ...
        public BindingNavigator BnKRTROMBOEKSTOGRAMM { get; set; }

        public void InitSQLData()
        {
            _db = new DataClassesLabDataContext();
            var res = (from c in _db.KRTROMBOEKSTOGRAMMs where c.pacient_id == PpacientID && c.otd == Potd select c);
            kRTROMBOEKSTOGRAMMBindingSource.DataSource = res;
            kRBIOHIMIIGridControl.DataSource = kRTROMBOEKSTOGRAMMBindingSource;
        ...
        private void BindingNavigatorAddNewItemClick(object sender, EventArgs e)
        {
            ...
            _kl = (KRTROMBOEKSTOGRAMM)gridView1.GetRow(sel);
            ...
            var frm = new FrmKrTromboex(kRTROMBOEKSTOGRAMMBindingSource) {Llabanaliz = Llaboranth};
            frm.Text += "  " + PFIO;
            frm.InitLookup();
        ...
        public void InsertOrder(KRTROMBOEKSTOGRAMM o)
        {
            _db = new DataClassesLabDataContext();
            _db.KRTROMBOEKSTOGRAMMs.InsertOnSubmit(o);
            try
            {
                _db.SubmitChanges(ConflictMode.ContinueOnConflict);
            }
        ...
        {
            _kl = (KRTROMBOEKSTOGRAMM)kRTROMBOEKSTOGRAMMBindingSource.Current;
            if (_kl == null) return;
            var frm = new FrmKrTromboex(kRTROMBOEKSTOGRAMMBindingSource) {Llabanaliz = Llaboranth};
            frm.Text += "  " + PFIO;
            ...
            else kRTROMBOEKSTOGRAMMBindingSource.CancelEdit();
        }
    }
}
