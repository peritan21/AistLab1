...
{
    public partial class UMochaSulkovich : UserControl
    {
        private MOCHASULKOVICH _kl;
        private DataClassesLabDataContext _db;
        public UMochaSulkovich()
        {
            InitializeComponent();
        }
        ...
        public BindingNavigator BnMOCHASULKOVICH { get; set; }

        public void InitSQLData()
        {
            _db = new DataClassesLabDataContext();
            var res = (from c in _db.MOCHASULKOVICHes where c.pacient_id == PpacientID && c.otd == Potd select c);
        ...
        private void BindingNavigatorAddNewItemClick(object sender, EventArgs e)
        {
            ...
            _kl = (MOCHASULKOVICH)gridView1.GetRow(sel);
            _kl.data = DateTime.Now;
            _kl.datatek = DateTime.Now;
            _kl.pacient_id = PpacientID;
            _kl.laborant_id = PlaborantID;
            _kl.otd = Potd;
            var frm = new FrmMohaSulkovich(mOHASAHARBELOKBindingSource) {Llabanaliz = Llaboranth};
            frm.Text += "  " + PFIO;
            frm.InitLookup();
        ...
        public void InsertOrder(MOCHASULKOVICH o)
        {
            _db = new DataClassesLabDataContext();
            _db.MOCHASULKOVICHes.InsertOnSubmit(o);
            try
            {
                _db.SubmitChanges(ConflictMode.ContinueOnConflict);
            }
        ...
        {
            // Редактирование 
            _kl = (MOCHASULKOVICH)mOHASAHARBELOKBindingSource.Current;
            if (_kl == null) return;
            var frm = new FrmMohaSulkovich(mOHASAHARBELOKBindingSource) {Llabanaliz = Llaboranth};
            frm.Text += "  " + PFIO;
