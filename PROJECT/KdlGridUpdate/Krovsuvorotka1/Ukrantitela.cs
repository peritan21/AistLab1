...
{
    public partial class Ukrantitela : UserControl
    {
        private KRANTITELA _kl;
        ...
        public void InitSQLData()
        {
            ...
            var res = (from c in _db.KRANTITELAs where c.pacient_id == PpacientID && c.otd == Potd select c);
        ...
        {
            // Редактирование
            _kl = (KRANTITELA)kRSUVIFAXGCHBindingSource.Current;
            if (_kl == null) return;
            var frm = new FrmKrantitelaigmigg(kRSUVIFAXGCHBindingSource) { Llabanaliz = Llaboranth };
            frm.Text += "  " + PFIO;
        ...
        private void BindingNavigatorAddNewItemClick(object sender, EventArgs e)
        {
            ...
            _kl = (KRANTITELA)gridView1.GetRow(sel);
        ...
        public void InsertOrder(KRANTITELA o)
        {
            _db = new DataClassesLabDataContext();
            _db.KRANTITELAs.InsertOnSubmit(o);
            try
            {
                _db.SubmitChanges(ConflictMode.ContinueOnConflict);
            }
