...
namespace KdlGridUpdate.Krovsuvorotka1
{
    public partial class Ukrantigominiz : UserControl
    {
        ...
        public void InitSQLData()
        {
            ...
            var res = (from c in _db.KRANTIGOMINIZs where c.pacient_id == PpacientID && c.otd == Potd select c);
        ...
        {
            // Редактирование
            _kl = (KRANTIGOMINIZ)kRSUVIFAXGCHBindingSource.Current;
            if (_kl == null) return;
            var frm = new FrmKrantihominiz(kRSUVIFAXGCHBindingSource) { Llabanaliz = Llaboranth };
            frm.Text += "  " + PFIO;
        ...
        private void BindingNavigatorAddNewItemClick(object sender, EventArgs e)
        {
            ...
            _kl = (KRANTIGOMINIZ)gridView1.GetRow(sel);
        ...
        public void InsertOrder(KRANTIGOMINIZ o)
        {
            _db = new DataClassesLabDataContext();
            _db.KRANTIGOMINIZs.InsertOnSubmit(o);
            try
            {
                _db.SubmitChanges(ConflictMode.ContinueOnConflict);
            }
