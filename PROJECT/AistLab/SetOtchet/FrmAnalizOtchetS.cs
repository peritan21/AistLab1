...
{
    public partial class FrmAnalizOtchetS : DevExpress.XtraEditors.XtraForm
    {
        private AccessorLab.OTCHETPERIODR_R _kle;
        private readonly DataClassesLabDataContext  _dt = new DataClassesLabDataContext();
        private readonly List<AccessorLab.OTCHETPERIODR_R> _lanalizr = new List<AccessorLab.OTCHETPERIODR_R>();
        ...
        private void TreeList2FocusedNodeChanged(object sender, FocusedNodeChangedEventArgs e)
        {
            if (e.Node != null)
            {
                _kle = (AccessorLab.OTCHETPERIODR_R)aNLOTCHETTREEBindingSource[treeList2.FocusedNode.Id];
                if (_kle != null)
                {
        ...
        private void LoadZapros()
        {
           ...
           string colamb_;
            string colstac;
            int? kol=0;
            var res11 = _dt.OTCHETPERIOD0NEW(Pdatevibstr,ref kol );
            _lanalizr.Clear();
            ...
            foreach (var t in res11)
            {
                ...
                _lanalizr.Add(new AccessorLab.OTCHETPERIODR_R(analizotchID, analizLevel
              , parentID
              , tabindex
             , analizID
            , nameStr
